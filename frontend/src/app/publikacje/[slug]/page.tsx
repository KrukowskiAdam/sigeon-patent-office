...
import { getPublicationBySlug, getPublicationsPageSettings } from '@/lib/queries'
...
export default function PublicationPage({ params }: Props) {
  ...
  const [publicationsSettings, setPublicationsSettings] = useState<Awaited<ReturnType<typeof getPublicationsPageSettings>> | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const load = async () => {
      try {
        const [publicationData, settingsData] = await Promise.all([
          getPublicationBySlug(resolvedParams.slug),
          getPublicationsPageSettings()
        ])
        setPublication(publicationData)
        setPublicationsSettings(settingsData)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="py-12">
          <div className="max-w-7xl mx-auto px-4">Publikacja nie znaleziona.</div>
        </main>
        <Footer />
      </div>
    )
        ...
        <article className="max-w-7xl mx-auto px-4">
          ...
          {publication.content && (
            <div className="prose prose-lg max-w-none">
              {/* Placeholder for content - PortableText temporarily disabled */}
              <div className="text-gray-700">
                <p>Treść publikacji będzie wyświetlana po włączeniu obsługi PortableText.</p>
              </div>
            </div>
          )}
