import { PublicationsPage, Publication } from '@/types/sanity'
...
interface PublicationsPageComponentProps {
  page: PublicationsPage
  currentLanguage: Language
}

export function PublicationsPageComponent({ page, currentLanguage }: PublicationsPageComponentProps) {
  ...
  return (
    <div>
                      ...
                      <div className="md:w-1/3 flex items-center justify-center bg-gray-50 p-8">
                        <div className="relative w-[80%] aspect-video overflow-hidden">
                          <Image
                            ...
                            alt={getLocalizedNewsText(publication.title, currentLanguage)}
                            fill
                            sizes="(max-width: 768px) 100vw, (max-width: 1024px) 33vw, 25vw"
                            className="object-cover"
                            priority={index === 0}
                            loading={index === 0 ? 'eager' : 'lazy'}
                            onError={() => console.error('Error loading image for publication:', getLocalizedNewsText(publication.title, currentLanguage))}
                          />
                        </div>
                      </div>
                    ) : (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <CardTitle className="text-lg md:text-xl text-gray-900 leading-tight">
                              {getLocalizedNewsText(publication.title, currentLanguage)}
                            </CardTitle>
                          </div>
                        </div>
      ...
      {page.socialSharing?.enableSharing && (
        <div className="bg-gray-50 py-8">
          <div className="max-w-6xl mx-auto px-4 text-center">
            {page.socialSharing.shareText && (
              <p className="text-gray-600 mb-6">
                {getLocalizedText(page.socialSharing.shareText, currentLanguage)}
              </p>
            )}
            {/* Social buttons implementation */}
            <div className="flex justify-center space-x-4">
              {page.socialSharing.platforms?.facebook && (
                <button className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                  Facebook
                </button>
              )}
              {page.socialSharing.platforms?.twitter && (
                <button className="bg-blue-400 text-white px-4 py-2 rounded hover:bg-blue-500">
                  Twitter
                </button>
              )}
              {page.socialSharing.platforms?.linkedin && (
                <button className="bg-blue-800 text-white px-4 py-2 rounded hover:bg-blue-900">
                  LinkedIn
                </button>
              )}
              {page.socialSharing.platforms?.email && (
